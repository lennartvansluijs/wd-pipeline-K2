...
def get_pixelpixflux (starname, filename, inputfolder='',outputfolder='', remove = True):
    ...
    if remove is True:
        # 16384 is not removed, because Kepler release says data point are not adversly affected
        flags = np.array([1,2,4,8,16,32,64,128,256,1024,40960,8192,32768,65536,12288,98304,131072,262144,524288,1089568, 1081376,1056768,1048576,1081344],dtype='int')# flags to be removed
...
def get_aperture(inputfolder, outputfolder, filename, starname, pixel_lim = 25, plot = True, intermediate_plot = False):
    ...
    # get raw data
    raw_time, raw_pixflux, magnitude, X_s, Y_s, campaign = get_pixelpixflux(starname, filename, inputfolder, outputfolder, remove = True)
    ...
    while N_pixel <= pixel_lim:
        ...
        if intermediate_plot is True:
            ...
            # create a plot of combined flux and aperture
            plt.imshow(flux, cmap = 'Greys', interpolation = 'none')
            plt.colorbar()
            plt.plot(outline[:, 0], outline[:, 1],color='b', zorder=10, lw=2.5)#,label=str(kepmag))
        ...
        bgflux, pixflux, time = get_backgroundflux(pixflux, aperture, time, outputfolder, starname, campaign, plot = False)
        ...
        time, flux, Xcof, Ycof = remove_poyntingjitter(time, flux, Xcof, Ycof, outputfolder, starname, plot = False)
    ...
    if plot is True:
            ...
            outline = make_aperture_outline(best_aperture) 
            plt.figure()
            ax = plt.gca()
            im = ax.pcolor(raw_flux, cmap = 'Greys')
            plt.plot(outline[:, 0]+0.5, outline[:, 1]+0.5,color='r', zorder=10, lw=2.5)#,label=str(kepmag))
            plt.xlabel('X', size = 17)
            plt.ylabel('Y', size = 17)
            #plt.title(str(starname), size =12)
            #plt.xlim(0,best_aperture.shape[1]-0.5)
            #plt.xticks(np.linspace(0,best_aperture.shape[1]-0.5,10))
            #plt.ylim(0,best_aperture.shape[0]-0.5)
            divider = make_axes_locatable(ax)
            cax = divider.append_axes("right", size="5%", pad=0.05)
            plt.colorbar(im, cax=cax)
            cax.set_ylabel('Counts', size = 17)
            plt.tight_layout()
            plt.savefig(outputfolder + '/aperture_'+ str(starname)+'.pdf')
            plt.savefig(outputfolder + '/aperture_'+ str(starname), dpi =300)
            #plt.show()
            plt.close()
...
def get_flux (pixflux, aperture, starname, bgflux, outputfolder, time = '', plot = True):
    ...
    if plot is True:
        ...
        plt.scatter(time, flux, color = 'k', s=0.75)
        ...
        #plt.savefig(outputfolder + '/lc_'+ str(starname)+'.pdf')
...
def get_aperture_of_fixed_size(time, pixflux, X_s, Y_s, inputfolder, outputfolder, filename, starname, aper_size, pixel_lim = 25, plot = True, intermediate_plot = False):
    ...
    if plot is True:
        ...
        plt.figure()
        ax = plt.gca()
        ax.imshow(raw_flux, cmap = 'Reds_r', interpolation = 'none')
        plt.colorbar()
        plt.plot(outline[:, 0], outline[:, 1],color='b', zorder=10, lw=2.5)#,label=str(kepmag))
        plt.xlabel('X', size = 12)
        plt.ylabel('Y', size = 12)
        ...
        cax = divider.append_axes("right", size="5%", pad=0.05)
        plt.colorbar(im, cax=cax)
        cax.set_ylabel('Coverage', size = 12)
        #plt.savefig(outputfolder + '/aperture_'+ str(starname), dpi =300)
        #plt.show()
        plt.close()
...
def from_pixel_to_flux (starname, filename, inputfolder, outputfolder):
    """
    ...
    aperture = get_aperture(inputfolder, outputfolder, filename, starname)
    
    # save best aperture
    np.save(os.path.join(outputfolder, 'aperture_' + starname), aperture)
    ...
    time, flux, pixflux = remove_time_range(time, flux, pixflux, t_min = 2988.19, t_max = 2988.56) # activate this for campaign 13 in between x1 = 2988.56 and x2 = 2988.19 to improve upon the data reduction
