...
def get_centerofflux(pixflux, time, aperture, outputfolder, starname, plot = False):
    ...
    if plot is True:
        ...
        plt.scatter(Xcof, Ycof, c=time, cmap = 'Reds')
        cbar = plt.colorbar()
...
def remove_thrusteronpoints(time, flux, Xcof, Ycof, outputfolder, starname, cutoff = 3.5, no_iter = 5, dr=5, plot = True):
    """
    ...
    if plot is True:
        ...
        ax2 = plt.subplot(gs[1:2,0:4])
        ax3 = plt.subplot(gs[2:3,0:4])
        ax4 = plt.subplot(gs[3:4,0:4])
        
        ax1.scatter(time, flux, color = 'k', label = 'removed', s = 0.5)
        ax1.set_xlabel('Time [BKJD]', size = 12)
        ax1.set_ylabel('Relative flux', size = 12)
        
        ax2.plot(time, Xcof, color = 'k', label = 'removed')
        ax2.set_xlabel('Time [BKJD]', size = 12)
        ax2.set_ylabel('Xcof', size = 12)
        
        ax3.plot(time, Ycof, color = 'k', label = 'removed')
        ax3.set_xlabel('Time [BKJD]', size = 12)
        ax3.set_ylabel('Ycof', size = 12)
        
        dist_cof = np.sqrt( np.diff(Xcof)**2 + np.diff(Ycof)**2 )
        ax4.plot(time[:-1]+np.diff(time), dist_cof, color = 'k', label = 'removed')
        ax4.set_xlabel('Time [BKJD]', size = 12)
        ax4.set_ylabel('Distance', size = 12)
        
        plt.savefig(outputfolder + '/xcofycof_'+ str(starname), dpi =300)
        #plt.show()
        plt.close()
...
def remove_outliers_for_fit(x, y, nbins = 5, cutoff = 3):
    """
...
def remove_poyntingjitter(time, flux, Xcof, Ycof, outputfolder, starname, noslices = 11, 
                          noiterations = 3, nbins = 11, plot = True, 
    ...
    for n in range(noiterations):
        ...
        for i in range(noslices):
            ...
            if plot_intermediate_step is True:
                ...
                plt.legend(loc = 4)            
                plt.xlabel('Deprojected x-coordinate of center of flux', size = 12)
                plt.ylabel('Flux (arbitrary units)', size = 12)
                plt.savefig(outputfolder + '/slice_' + str(i) + '_fluxcentroid_'+ str(starname), dpi =300)
                plt.close()
    ...
    if robust is True:
        ...
        for i in range(noslices):
            if std_sliced[i] > 2 * std:
                ...
                t_min_robust = time[indices[i_min]]
                t_max_robust = time[indices[i]] if i < len(indices) else time[-1]
